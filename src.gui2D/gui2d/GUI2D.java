...
public class GUI2D extends SimpleApplication implements PokemonGameView {
  ...
  public ArrayList<Card> userChoosesCards(List<Card> cards, int amount, boolean exact, String message) {
    ...
    /*
     * No need to wait for update queue or game model update, since waiting for update queue is
     * being done by #addToUpdateQueue and the game model is irrelevant for card choose models.
     */
  ...
  public List<PositionID> userChoosesPositions(List<PositionID> positionList, int amount, boolean exact, String message) {
    ...
    /*
     * Currently only one position is being chosen here. This is valid, since no card script in this
     * game forces the player to select multiple positions at once. If there is a card added to the
     * game that does this, this method has to be rewritten.
     */
    ...
    final SelectableNode node = this.ingameController.getCurrentlySelected(); // Node that was
                                                                              // selected
  ...
  public void setCardChoosable(Position position, int i, boolean value) {
    // System.out.println("SetCardChoosable for (" + position.getPositionID() + ", " + i + ")" + "
    ...
    if (node instanceof HandCardManager2D) {
      // It may happen that the hand card manager is not ready for being updated, so we have to wait
      // until he is.
