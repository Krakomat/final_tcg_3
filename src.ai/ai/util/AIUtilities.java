...
public class AIUtilities {
  ...
  /**
   * Computes all possible player actions that can be executed by the given player on the given game
   * model.
   * 
   * @param gameModel
   * @param player
          ...
          for (String action : actions) {
            if (action.equals(PlayerAction.PLAY_ENERGY_CARD.toString())) {
              for (PositionID arenaPosition : gameModel.getFullArenaPositions(Color.BLUE)) {
                ...
                choosableCards
                    .add(new GameTreeMove(new Triple<Position, Integer, String>(handPos, i, action), posQueue, new LinkedList<>(), new LinkedList<>(), new LinkedList<>()));
              }
            } else
  ...
  /**
   * Returns the indices of the given energy cards in the given list, if they are not needed to pay
   * the given costs.
   * 
   * @param attackCosts
   * @param energyCards
  ...
  public List<Integer> getUnneccesaryEnergyIndices(List<Element> attackCosts, List<Card> energyCards) {
    ...
    for (Card c : energyCards)
      availableCards.add(0, c); // Add in reverse, s.t. the oldest energy cards will be checked
                                // first!
