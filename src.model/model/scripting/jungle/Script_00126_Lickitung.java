...
public class Script_00126_Lickitung extends PokemonCardScript {

	public Script_00126_Lickitung(PokemonCard card, PokemonGame gameModel) {
		super(card, gameModel);
		List<Element> att1Cost = new ArrayList<>();
		att1Cost.add(Element.COLORLESS);
		this.addAttack("Tongue Wrap", att1Cost);

		List<Element> att2Cost = new ArrayList<>();
		att2Cost.add(Element.COLORLESS);
		att2Cost.add(Element.COLORLESS);
		this.addAttack("Supersonic", att2Cost);
	}

	@Override
	public void executeAttack(String attackName) {
		if (attackName.equals("Tongue Wrap"))
			this.tongueWrap();
		else
			this.supersonic();
	}

	private void tongueWrap() {
