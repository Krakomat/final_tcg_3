...
public class PokemonGameModelImpl implements PokemonGame {
  ...
  /**
   * Simulates the initial draw phase. Both players draw 7 cards. If a player doesn't have any basic
   * pokemon in his hand, he shuffles his hand into his deck and draws once more.
   */
  ...
  private void startupCoinflip() {
    ...
    switch (coinflip) {
      case HEADS:
        this.playerOnTurn = playerRed; // Gets switched before the real first turn, because of
                                       // method nextTurn()!
        this.sendTextMessageToPlayers(getPlayerList(), "Coin shows 'Heads' - " + playerBlue.getName() + " begins.", "");
        break;
      case TAILS:
        this.playerOnTurn = playerBlue; // Gets switched before the real first turn, because of
                                        // method nextTurn()!
  ...
  /**
   * Cleans all positions with defeated pokemon on it. Also forces players to take prices and checks
   * the winning conditions. Does nothing if the given list is empty.
   * 
  ...
  /**
   * Checks the arena field for defeated pokemon and adds all found positions to a list, which is
   * returned.
   *
   * @return
   */
  ...
  /**
   * Checks if any of both player lost the game, sets and returns the {@link GameState}, which has
   * to occur.
   * 
   * @return
   */
  ...
  /**
   * Switches the turn and updates the gameModel for both players. Starts the next turn, by
   * initiating the draw phase.
   */
  ...
  /**
   * Makes actions that have to be executed between the turns, e.g. applying poison damage. Does
   * nothing, if there is nothing to do in between turns.
   */
  ...
  /**
   * Creates an instance of the given deck, by creating instances of the cards of the deck and
   * assigning a gameID to them.
   * 
   * @param deck
   * @return
   */
  ...
  /**
   * Searches for the card with the given gameID and returns it. Returns null if no such card was
   * found.
   * 
   * @param cardGameID
   * @return
