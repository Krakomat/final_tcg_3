...
/**
 * This class is responsible for creating new games and managing all current
 * games.
 * 
 * @author Michael
 *
...
public class PokemonGameManagerFactory {
	...
	/**
	 * Creates a new {@link PokemonGameManager} and returns its id to the method
	 * caller.
	 * 
	 * @param serverMain
	 * 
	 ...
	 * @return
	 */
	public static PokemonGameManager createNewGame(String name, String password) {
		long minIDMissing = 1L;
		for (Long k : GAME_MAP.keySet())
			if (k.longValue() == minIDMissing)
				minIDMissing++;
		// Create new instance:
		PokemonGameManagerImpl pkmnGameMngr = new PokemonGameManagerImpl(minIDMissing, name, password);
		GAME_MAP.put(minIDMissing, pkmnGameMngr);
	...
	/**
	 * Returns the instance of the {@link PokemonGameManager} with the given id.
	 * Returns null, if no such instance with the id was found.
	 * 
	 * @param id
	 * @return
	 */
